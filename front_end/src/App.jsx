...
// --- COLOR PALETTE ---
const COLORS = {
  PRIMARY: '#A22829',      // brown-red
  PRIMARY_ALT: '#9C2A2B',  // brown-red-2
  ACCENT: '#C05E5A',       // lobster-pink
  SECONDARY: '#9E2729',    // brown-red-3
  DARK: '#9D2627'          // brown-red-4
};

// --- COMPONENTS ---

// 1. Navigation Bar (Brown-Red Theme)
const Navbar = ({ activeTab, setActiveTab }) => (
  <nav className="sticky top-0 z-50 w-full bg-black border-b-4 shadow-xl" style={{ borderColor: COLORS.PRIMARY }}>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between h-20">
        ...
        >
          <div className="p-1.5 transition-transform group-hover:rotate-180 duration-500" style={{ backgroundColor: COLORS.PRIMARY }}>
             <Shield className="h-8 w-8 text-white" />
          </div>
          <span className="font-black text-3xl tracking-tighter text-white">
            BA7ATH<span style={{ color: COLORS.PRIMARY }}>.TN</span>
          </span>
        </div>
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className={`px-6 py-2 text-sm font-black uppercase tracking-widest transition-all duration-200 border-2 ${
              activeTab === 'upload' 
                ? 'text-white border-2' 
                : 'text-white border-transparent hover:border-opacity-50'
            }`}
            style={{
              backgroundColor: activeTab === 'upload' ? COLORS.PRIMARY : 'transparent',
              borderColor: activeTab === 'upload' ? COLORS.PRIMARY : 'transparent'
            }}
          >
            Verify
          </button>
...
// 2. Result Card Component (High Contrast Brown-Red/Black)
const ResultCard = ({ result }) => {
  if (!result) return null;

  // Determine styles based on verdict
  // If "Fake" or "Tampering", use brown-red. If "Real", use BLACK (instead of Green).
  const isFake = result.analysis?.label === 'Fake' || result.analysis?.verdict === 'Tampering Detected';
  const headerBg = isFake ? COLORS.PRIMARY : 'bg-black';
  const accentText = isFake ? COLORS.PRIMARY : 'black';
  const barColor = isFake ? COLORS.PRIMARY : 'black';
  ...
  return (
    <div className="bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] border-4 mt-8 mb-8" style={{ borderColor: isFake ? COLORS.PRIMARY : 'black' }}>
      {/* Header */}
      <div className="px-6 py-4 flex justify-between items-center text-white" style={{ backgroundColor: isFake ? COLORS.PRIMARY : 'black' }}>
      ...
      <div className="p-6 md:p-8">
        ...
        {result.type === 'text' ? (
          ...
          <div className="space-y-8">
            <div>
              <p className="text-xs font-bold uppercase mb-2" style={{ color: COLORS.PRIMARY }}>Analyzed Statement</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="border-2 border-black p-6 relative">
                ...
                <div className="flex items-baseline gap-2 mb-4">
                  <span className="text-6xl font-black" style={{ color: accentText }}>{result.analysis.confidence}%</span>
                </div>
                <div className="w-full bg-gray-200 h-4 border-2 border-black">
                  <div className="h-full transition-all duration-1000" style={{ width: `${result.analysis.confidence}%`, backgroundColor: barColor }}></div>
                </div>
                {meta && (
                    ...
                    {meta.engines_agree !== null && (
                      <div className="flex items-center gap-1">
                        {meta.engines_agree ? (
                          ...
                          <span style={{ color: COLORS.PRIMARY }} className="font-bold">Engines conflict – treat this result with caution.</span>
                        )}
                      </div>
                    )}
                  </div>
              ...
              <div className="space-y-4">
                <div>
                  <span className="text-xs font-bold uppercase mb-2 block" style={{ color: COLORS.PRIMARY }}>AI Reasoning</span>
                ...
                {mlModel && (
                  <div className="border-2 border-black p-4 bg-gray-50">
                    ...
                    {enginesDisagree && (
                      <div className="mt-3 flex items-center gap-2 text-[11px] font-bold" style={{ color: COLORS.PRIMARY }}>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <p className="text-xs font-bold uppercase mb-2" style={{ color: COLORS.PRIMARY }}>Metadata Forensics</p>
                ...
                <div className="mt-6">
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-4xl font-black" style={{ color: accentText }}>
                      {result.analysis.authenticity_score}/100
                    </span>
                  </div>
                  <div className="w-full bg-gray-200 h-4 border-2 border-black">
                    <div className="border-r-2 border-white" style={{ width: `${result.analysis.authenticity_score}%`, backgroundColor: barColor }}></div>
                  </div>
                </div>
              ...
              <div>
                 <p className="text-xs font-bold uppercase mb-2" style={{ color: COLORS.PRIMARY }}>Tamper Map Visualization</p>
...
// 3. Main Upload Page (Brown-Red Tabs & Inputs)
    ...
    <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="text-center mb-10">
        <h1 className="text-5xl md:text-6xl font-black text-black mb-4 uppercase italic tracking-tighter">
          Verify <span style={{ color: COLORS.PRIMARY }}>Everything</span>
        </h1>
      ...
      <div className="bg-white border-4 border-black shadow-lg" style={{ boxShadow: `12px 12px 0px 0px ${COLORS.ACCENT}40` }}>
        {/* Tabs */}
          ...
          <form onSubmit={handleSubmit}>
            {activeMode === 'text' ? (
              <div className="space-y-4 animate-fade-in">
                ...
                <textarea
                  rows={6}
                  className="block w-full text-lg font-medium border-4 border-gray-200 p-4 resize-none bg-white text-black placeholder-gray-300 focus:outline-none focus:ring-0 transition-colors"
                  style={{
              ...
              <div className="space-y-4 animate-fade-in">
                ...
                <div className="mt-1 flex justify-center px-6 pt-12 pb-12 border-4 border-gray-200 border-dashed bg-gray-50 cursor-pointer group relative transition-colors" style={{ borderColor: '#E5E7EB' }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = COLORS.PRIMARY; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = '#E5E7EB'; }}>
                  <div className="space-y-4 text-center">
                    <div className="mx-auto h-16 w-16 text-gray-300 group-hover:transition-colors" style={{ color: '#D1D5DB' }}>
                      <Upload className="h-16 w-16" />
                    </div>
                    <div className="flex text-sm text-gray-600 justify-center">
                      <label htmlFor="file-upload" className="relative cursor-pointer font-bold text-black uppercase tracking-wide focus-within:outline-none" style={{ color: 'black' }}>
            ...
            {error && (
              <div className="mt-6 p-4 text-white font-bold text-sm flex items-center gap-3" style={{ backgroundColor: COLORS.PRIMARY }}>
            ...
            <div className="mt-10">
              <button
                type="submit"
                disabled={loading}
                className="w-full flex justify-center py-5 px-4 border-4 border-transparent font-black uppercase tracking-[0.2em] text-lg transition-all shadow-lg hover:shadow-none disabled:opacity-50 disabled:cursor-not-allowed group"
                style={{
...
export default function App() {
  ...
  return (
    <>
      {/* MAIN RENDER LOGIC */}
      <div className="min-h-screen w-full bg-white font-sans" style={{ 
